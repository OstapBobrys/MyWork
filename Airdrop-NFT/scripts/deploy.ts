...
import { Airdrop__factory, BonitaNFT__factory } from '../../typechain-types'

async function main() {
  const [signer] = await ethers.getSigners()

  const token = await new BonitaNFT__factory(signer).deploy()
  ...
  await run('verify:verify', {
    ...
    contract: 'contracts/airdrop/BonitaNFT.sol:BonitaNFT'
  })
